/**
 * Liquid Background Component - MOOD-AWARE COLOR REACTIVE 🎨
 * Refined version with AI mood detection + smooth liquid-like particle flow
 * - AI analyzes video title to detect mood (romantic, energetic, chill, etc.)
 * - 18 rich holographic presets for different moods
 * - Smooth 3-second transitions between palettes
 * - Organic flowing particle movement with liquid dynamics
 * - 📱 Mobile optimized with reduced effects
 * - variant="classic" (spirals) or variant="orbital" (circular orbits)
 */
...
const DEFAULT_COLORS = [
  { r: 212, g: 165, b: 116 },   // Primary gold #d4a574
  { r: 180, g: 120, b: 80 },    // Deep amber/copper
  { r: 230, g: 184, b: 136 },   // Light gold #e6b888
  { r: 160, g: 100, b: 60 },    // Warm brown
  { r: 200, g: 150, b: 100 },   // Soft amber
]
...
const Liquid = ({ 
  ...
  videoTitle = null,
  channelName = null,
  variant = 'classic', // 'classic' (spirals) or 'orbital' (circular)
  tvFrameRect = null   // { x, y, width, height } - TV frame position for orbital variant
}) => {
  ...
  const isActiveRef = useRef(isActive) // Ref to track isActive in animation loop
  const [performanceSettings, setPerformanceSettings] = useState(() => mobilePerformanceOptimizer.getSettings())
  
  // ═══════════════════════════════════════════════════════════════════
  // SHARED COLOR STATE - Used by both classic and orbital variants
  // ═══════════════════════════════════════════════════════════════════
  const [colors, setColors] = useState(DEFAULT_COLORS)
  const [intensityBoost, setIntensityBoost] = useState(0.7)
  
  // Refs for smooth animation transitions (internal use)
  ...
  const intensityBoostRef = useRef(0.7)
  ...
  // ═══════════════════════════════════════════════════════════════════
  // COLOR UPDATE HANDLER - Shared between both variants
  // ═══════════════════════════════════════════════════════════════════
  const updateColors = useCallback((newColors, newIntensity = 0.7) => {
    if (newColors && newColors.length >= 3) {
      // Boost saturation for more vibrant effect
      const boostedColors = newColors.slice(0, 5).map(c => {
        const avg = (c.r + c.g + c.b) / 3
        return {
          r: Math.min(255, Math.round(c.r + (c.r - avg) * 0.2)),
          g: Math.min(255, Math.round(c.g + (c.g - avg) * 0.2)),
          b: Math.min(255, Math.round(c.b + (c.b - avg) * 0.2)),
        }
      })
      ...
      // Update refs for classic variant animation
      targetColorsRef.current = boostedColors
      colorTransitionRef.current = 0
      intensityBoostRef.current = newIntensity
      
      // Update state for orbital variant (triggers re-render)
      setColors(boostedColors)
      setIntensityBoost(newIntensity)
      
      console.log('[Galaxy] 🎨 Colors updated:', boostedColors.slice(0, 2).map(c => `rgb(${c.r},${c.g},${c.b})`))
    }
  }, [])

  // Subscribe to mood color service for AI-detected moods
  useEffect(() => {
    const unsubscribe = moodColorService.subscribe((preset, mood) => {
      if (preset?.colors) {
        updateColors(preset.colors, preset.intensity || 0.7)
        console.log(`[Galaxy] 🎨 Mood: ${mood} → ${preset.name}`)
      }
    })
    return unsubscribe
  }, [updateColors])

  // Analyze video mood when video changes
  useEffect(() => {
    if (videoId && videoTitle) {
      moodColorService.setMoodFromVideo(videoId, videoTitle, channelName)
    }
  }, [videoId, videoTitle, channelName])

  // Extract colors from video thumbnail (fallback when no AI mood)
  useEffect(() => {
    if (!videoId) return
    // If we have a title, mood service will handle it
    ...
    const extractColors = async () => {
      try {
        ...
        if (sortedColors.length >= 3) {
          // Use shared updateColors function
          updateColors(sortedColors, 0.7)
        }
      } catch (err) {
    ...
    extractColors()
  }, [videoId, updateColors])

  useEffect(() => {
    // Skip canvas setup for orbital variant (it has its own component)
    if (variant === 'orbital') return
    ...
    const animate = (timestamp) => {
      // Skip if galaxy disabled for performance (check ref for live value)
      ...
      // Intensity builds over ~30 second cycle, boosted by mood
      const moodBoost = intensityBoostRef.current
  ...
  }, [baseSpeed, density, isPlaying, volume, performanceSettings.enableGalaxy, performanceSettings.animationFPS, variant])
